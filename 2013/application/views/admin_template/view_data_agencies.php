	...
	<ul class="nav">
	  ...
	  <li><?php echo anchor('backoffice/agencies/create', '<i class="icon-book"></i> Create Agencies'); ?></li>
	  <li class="divider-vertical"></li>
	...
	<?php echo form_open('backoffice/search' , array('method'=>'get', 'class' => 'navbar-form pull-right')); ?>
	    <input type="text" name="word" class="input-large" placeholder="Search Info">
	...
	$query = $this->db->query("SELECT a.*, b.name_th FROM {$this->db->dbprefix('agencies')} a
						  LEFT JOIN {$this->db->dbprefix('department')} b ON a.dep_id = b.id
						  WHERE b.division='{$this->input->get('group')}'
						");
	
	$sortby = (isset($_GET['sortBy']))? $_GET['sortBy'] : 'a.dep_id , a.created';
        $result = $this->db->query("SELECT a.*, b.name_th FROM {$this->db->dbprefix('agencies')} a
						  LEFT JOIN {$this->db->dbprefix('department')} b ON a.dep_id = b.id
						  WHERE b.division='{$this->input->get('group')}'
    ...
    <thead>
        <tr>
            ...
            <th class="table-header-repeat"><a href="?group=<?php echo $this->input->get('group'); ?>&sortBy=a.agen_name">ชื่อบุคลากร</a></th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?php echo image_asset("agencies/thumb/{$row->agen_img}", 'control', array('width'=>'48')); ?><?php echo $row->agen_name; ?></td>
            ...
            <div class="btn-group">
        	<a class="btn btn-inverse" href="<?php echo $this->Option_model->getURL( 'agencies/edit', array('editId'=>$row->id, 'group'=>$this->input->get('group'))); ?>"><i class="icon-cog icon-white"></i> Quick Setting </a>
		...
		<li class="divider"></li>
                ...
                <li><a href="<?php echo $this->Option_model->getURL( "chkActiveItembyGET", array('activeId'=>$row->id,'is_control'=>$this->controller, 'is_view'=>$this->view, 'is_table'=>'agencies', 'is_group'=>$this->input->get('group'), 'status'=>$status)); ?>"><?php echo $icon; ?><?php echo $active; ?></a></li>
              </ul>
        </div>
            </td>
        </tr>
