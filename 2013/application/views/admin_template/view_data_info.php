	...
	<ul class="nav">
	  ...
	  <li><?php echo anchor('backoffice/info/create', '<i class="icon-book"></i> Create info'); ?></li>
	  <li class="divider-vertical"></li>
	...
	$query = $this->db->query("SELECT a.*, b.name_th FROM {$this->db->dbprefix('info')} a
	...
	$sortby = (isset($_GET['sortBy']))? $_GET['sortBy'] : 'a.dep_id , a.created';
        $result = $this->db->query("SELECT a.*, b.name_th FROM {$this->db->dbprefix('info')} a
						  LEFT JOIN {$this->db->dbprefix('department')} b ON a.dep_id = b.id
						  WHERE b.division='{$this->input->get('group')}'
    ...
    <tbody>
        ...
        <tr>
            ...
            <div class="btn-group">
        	<a class="btn btn-inverse" href="<?php echo $this->Option_model->getURL( 'info/edit', array('editId'=>$row->id, 'group'=>$this->input->get('group'))); ?>"><i class="icon-cog icon-white"></i> Quick Setting </a>
		...
		<li class="divider"></li>
                ...
                <li><a href="<?php echo $this->Option_model->getURL( "chkActiveItembyGET", array('activeId'=>$row->id,'is_control'=>$this->controller, 'is_view'=>$this->view, 'is_table'=>'info', 'is_group'=>$this->input->get('group'), 'status'=>$status)); ?>"><?php echo $icon; ?><?php echo $active; ?></a></li>
              </ul>
        </div>
            </td>
        </tr>
