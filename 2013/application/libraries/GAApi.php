 ...
 */
  ...
  class GAApi{
   ...
   public function __construct(){
		...
		$this->_sUser = $CI->config->item('ga_email');
        $this->_sPass = $CI->config->item('ga_password');
    ...
    */
    function auth(){
        ...
        if ($CI->session->userdata('auth') != ""){
            $this->_sAuth = $CI->session->userdata('auth');
            return;
        }
        ...
        if (strpos($sResponse, "\n") !== false){
            ...
            foreach ($aResponse as $sResponse){
                if (substr($sResponse, 0, 4) == 'Auth'){
                     $CI->session->set_userdata('auth', trim(substr($sResponse, 5)));
                }
            }
        }
    ...
    */
    public function useCache($bCaching = true, $iCacheAge = 600){
        ...
        if ($bCaching && $CI->session->userdata('cache') == ""){
            $CI->session->set_userdata('cache', array());     
        }
    }
    ...
    */
    public function setProfileByName($sAccountName){
        ...
        if ($CI->session->userdata('profile') != ""){
            $this->_sProfileId = $CI->session->userdata('profile');
            return;
        }
        ...
        $CI->session->set_userdata('profile', $this->_sProfileId);
    }
